...
achieves $\approx$74\% of QD's precision at $\approx$63--65\% of
QD's per-operation cost.
...
the per-operation cost is 62--65\% that of QD for the basic
arithmetic operations: TD$\times$TD requires 6 \textsc{TwoProd}
calls vs.\ QD's 10, and the 3-component renormalization is cheaper
than QD's 4-component variant.
...
each same-type addition and multiplication, derived by composing
the primitive costs from Tables~\ref{tab:eft-costs}
and~\ref{tab:renorm-costs}.  Since addition and subtraction use
only \textsc{TwoSum}/\textsc{TwoDiff}/\textsc{QuickTwoSum}
(all independent of FMA), FMA affects only the multiplication
columns.

\begin{table}[h]
\centering
\caption{Binary64 flop counts for DD, TD, and QD addition and
  multiplication.  \#TP denotes the number of \textsc{TwoProd} calls.
...
\midrule
TD$+$TD & ieee\_add (typical) & 55 & 55 & 0
  & $1\,\text{QTS}+3\,\text{QTA}+1+\text{renorm4}$ \\
TD$+$TD & ieee\_add (worst) & 68 & 68 & 0
  & $1\,\text{QTS}+4\,\text{QTA}+2+\text{renorm4}$ \\
TD$-$TD & direct & 58 & 58 & 0
  & $3\,\text{TD}+1\,\text{TS}+1\,\text{3S}+1+\text{renorm4}$ \\
TD$\times$TD & --- & 116 & 206 & 6
  & $6\,\text{TP}+3\,\text{3S}+3\,\text{TS}+8+\text{renorm5}$ \\
\midrule
QD$+$QD & sloppy (default) & 84 & 84 & 0
  & $5\,\text{TS}+1\,\text{3S}+1\,\text{3S2}+2+\text{renorm5}$ \\
QD$\times$QD & sloppy & 120 & 210 & 6
  & $6\,\text{TP}+3\,\text{3S}+3\,\text{TS}+12+\text{renorm5}$ \\
QD$\times$QD & accurate (default) & 182 & 332 & 10
...
Numerals without abbreviation denote plain add/mul flops.
\end{table}

Several observations are noteworthy:

\begin{itemize}
\item The TD$+$TD \texttt{ieee\_add} is data-dependent because the
  merge loop emits a resolved word only when the running pair
  $(u, v)$ in \textsc{QuickThreeAccum} is fully occupied.  In the
  typical case, 3 of the 4 remaining components emit immediately,
  giving 55~flops.  The worst case (4 iterations without early
  emission) costs 68~flops.

\item The TD$-$TD \texttt{direct} subtraction (58~flops) is
  slightly more expensive than the typical TD$+$TD (55~flops) but
  avoids the data-dependent branches entirely, giving deterministic
  performance.
...
\item TD$\times$TD and QD$\times$QD \texttt{sloppy\_mul} have the
  ...
  $O(\varepsilon^3)$ terms ($a_0 b_3$, $a_1 b_2$, $a_2 b_1$,
  $a_3 b_0$).  TD omits only the terms with $i+j > 2$.

\item FMA provides a $\approx 1.8\times$ speedup for multiplication
  (e.g., 116 vs.\ 206 for TD$\times$TD) but does not affect addition.
\end{itemize}

Table~\ref{tab:flop-ratio} gives the normalized cost ratios.  TD
achieves approximately 74\% of QD's precision (157 vs.\ 212 bits)
at approximately 63--65\% of QD's per-operation cost, confirming
...
\toprule
      & \multicolumn{3}{c}{Addition} &
        \multicolumn{3}{c}{Multiplication (FMA / No FMA)} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
Type  & Bits & Flops & Ratio & Flops & Ratio & TD/QD \\
\midrule
DD    & 106  & 11  & $1.0\times$  & 9 / 24   & $1.0\times$
  & --- \\
TD    & 157  & 55  & $5.0\times$  & 116 / 206 & $12.9\times$ / $8.6\times$
  & 65\% / 65\% \\
...
\\[2pt]
\footnotesize
TD/QD column shows TD cost as a percentage of QD cost:
addition $55/84 = 65\%$; multiplication $116/182 = 64\%$
(FMA), $206/332 = 62\%$ (no FMA).
\end{table}
...
74\%$ of QD's 63 digits) at 62--65\% of the per-operation cost of
quad-double arithmetic (Section~\ref{sec:cost}): TD addition
costs 55~flops vs.\ QD's~84, and TD multiplication costs 116~flops
(FMA) or 206~flops (no FMA) vs.\ QD's 182 or~332.  The
implementation follows the proven algorithmic framework of the QD
